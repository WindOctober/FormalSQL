...
Require Import BasicFacts ListFacts ListPermut ListSort OrderedSet FiniteSet Bool3. 
...
Definition union_max_ (s1 s2 : bag_) :=
  flat_map (fun a => mk_dup_list (N.to_nat (N.max (nb_occ_ a s1) (nb_occ_ a s2))) a) 
...
- case_eq (Oeset.mem_bool Elt a1 acc); intro Ha1; rewrite <- IHl.
  + case_eq ( Oeset.eq_bool Elt x a1); intro Hx; [ | apply refl_equal].
    ...
    apply refl_equal.
  + simpl; rewrite eq_bool_iff, 4 Bool.Bool.orb_true_iff; intuition.
Qed.
...
Lemma union_max_spec_ : 
  forall (s s' : bag_) (x : elt), nb_occ_ x (union_max_ s s') = N.max (nb_occ_ x s) (nb_occ_ x s').
Proof.
intros s1 s2 e.
unfold union_max_.
...
- generalize (sym_eq H2); rewrite Oeset.mem_bool_app; simpl.
  rewrite Bool.Bool.orb_false_iff; intros [K1 K2].
...
Lemma nb_occ_inter_ : 
  forall (s s' : bag_) (x : elt), nb_occ_ x (inter_ s s') = N.min (nb_occ_ x s) (nb_occ_ x s').
Proof.
intros s1 s2 x.
...
- destruct l1 as [ | a1 l1]; [simpl; apply refl_equal | rewrite diff_bag_equation].
  ...
  + rewrite IHm, 2 (Oeset.nb_occ_unfold _ _ (_ :: _)).
    ...
    * rewrite (Oeset.compare_eq_trans _ _ _ _ Hx Ha).
      {
        case_eq (N.compare (Oeset.nb_occ Elt x l1) (Oeset.nb_occ Elt x l2)); intro H.
...
intros l1 l2; split.
- do 2 intro; apply Oeset.permut_nb_occ.
...
Definition subbag_ l1 l2 :=
  forallb (fun x => match N.compare (Oeset.nb_occ Elt x l1) (Oeset.nb_occ Elt x l2) with
                      | Eq | Lt => true 
                      | Gt => false 
                    end) l1.
...
- intros x.
  ...
  + rewrite Oeset.mem_bool_true_iff in Hx.
    ...
    assert (Ka := H _ Ha).
    case_eq (N.compare (Oeset.nb_occ Elt a l1) (Oeset.nb_occ Elt a l2)); 
...
- intros x _; generalize (H x); unfold N.le.
  case (N.compare (Oeset.nb_occ Elt x l1) (Oeset.nb_occ Elt x l2)); trivial.
  intro Abs; apply False_rec; apply Abs; apply refl_equal.
Qed.
...
apply Oeset.permut_trans with l1.
- apply Oeset.permut_sym; apply quick_permut.
- apply Oeset.permut_trans with l2.
...
- rewrite nb_occ_equal_; unfold nb_occ_.
  intro; apply Oeset.permut_nb_occ.
  apply Oeset.permut_trans with (quicksort Elt s1); [apply quick_permut | ].
  apply Oeset.permut_trans with (quicksort Elt s2); [ | apply Oeset.permut_sym; apply quick_permut].
  ...
  + case_eq (Oeset.compare Elt a1 a2); intros Ha H; try discriminate H.
    apply Oeset.permut_cons; [ | apply IHl1]; assumption.
- unfold equal_ in H; unfold compare_.
  apply sort_is_unique.
  + apply quick_sorted.
  + apply quick_sorted.
  + apply Oeset.permut_trans with s2;  [ | apply quick_permut].
    apply Oeset.permut_trans with s1; [apply Oeset.permut_sym; apply quick_permut | ].
...
Definition cardinal b := N.of_nat (List.length (elements BA b)).

Lemma cardinal_eq :
  forall s1 s2, equal BA s1 s2 = true -> cardinal s1 = cardinal s2.
Proof.
intros s1 s2 Hs; unfold cardinal.
apply f_equal.
apply (comparelA_eq_length_eq _ _ _ (elements_spec1 _ _ _ Hs)).
Qed.

Lemma nb_occ_eq_1 :
  forall s x1 x2, Oeset.compare EA x1 x2 = Eq -> nb_occ BA x1 s = nb_occ BA x2 s.
Proof.
intros s x1 x2 Hx.
rewrite !nb_occ_elements.
apply Oeset.nb_occ_eq_1; assumption.
Qed.
...
Lemma equal_refl :
  forall s, equal BA s s = true.
Proof.
intro s; rewrite nb_occ_equal; intro; apply refl_equal.
Qed.
...
Lemma mem_nb_occ :
  forall a s, mem a s = match N.compare (nb_occ BA a s) 0 with
    | Lt | Eq => false
    | Gt => true
  end.
Proof.
...
Lemma cardinal_mk_bag :
  forall l, cardinal (mk_bag l) = N.of_nat (length l).
Proof.
intro l; unfold cardinal; apply f_equal.
apply _permut_length with (fun x y => Oeset.compare EA x y = Eq).
apply Oeset.nb_occ_permut; intro x.
rewrite <- nb_occ_elements, nb_occ_mk_bag; apply refl_equal.
Qed.
...
Lemma filter_true_alt :
  forall (B : Bool.Rcd) b, equal BA (filter BA (fun _ => Bool.is_true B (Bool.true B)) b) b = true.
Proof.
intros B b; rewrite nb_occ_equal.
intros a; rewrite nb_occ_filter; [ | intros; trivial].
assert (Aux := refl_equal (Bool.true B)).
rewrite <- Bool.true_is_true in Aux; rewrite Aux, N.mul_1_r; apply refl_equal.
Qed.
...
+ rewrite Bool.Bool.andb_false_iff in H2.
  ...
  * rewrite H2, N.mul_0_r in K.
    generalize (Oeset.mem_nb_occ _ _ _ H1); rewrite K.
    intro Abs; apply False_rec; apply Abs; apply refl_equal.
+ rewrite Bool.Bool.andb_true_iff in H2.
...
Lemma is_empty_eq :
 forall b1 b2, equal BA b1 b2 = true -> is_empty BA b1 = is_empty BA b2.
Proof.
intros b1 b2 H; rewrite nb_occ_equal in H.
rewrite eq_bool_iff; rewrite !is_empty_spec, !nb_occ_equal; split; intros K a.
- rewrite <- H; apply K.
- rewrite H; apply K.
Qed.

Lemma is_empty_union :
  forall b1 b2, is_empty BA (union BA b1 b2) = is_empty BA b1 && is_empty BA b2.
Proof.
intros b1 b2.
rewrite !is_empty_spec, eq_bool_iff, !Bool.Bool.andb_true_iff, !nb_occ_equal; repeat split.
- intro e; generalize (H e); rewrite nb_occ_union, nb_occ_empty.
  destruct (nb_occ BA e b1); [trivial | intro He; destruct (nb_occ BA e b2); discriminate He].
- intro e; generalize (H e); rewrite nb_occ_union, nb_occ_empty.
  destruct (nb_occ BA e b2); [trivial | intro He; destruct (nb_occ BA e b1); discriminate He].
- intros [H1 H2] e.
  rewrite nb_occ_union, (H1 e), (H2 e), nb_occ_empty; trivial.
Qed.
...
- apply Oeset.permut_nb_occ.
  ...
  + apply Oeset.nb_occ_permut; clear x; intro x.
