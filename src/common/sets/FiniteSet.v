...
Lemma mk_set_app :
  forall l1 l2, equal _ (mk_set (l1 ++ l2)) (union _ (mk_set l1) (mk_set l2)) = true.
Proof.
intros l1 l2; rewrite equal_spec; intro a.
rewrite mem_union, 3 mem_mk_set, Oeset.mem_bool_app.
apply refl_equal.
Qed.
...
Definition nb_occ e s := Oset.nb_occ Elt e (elements FA s).

Lemma nb_occ_unfold : forall e s, nb_occ e s = Oset.nb_occ Elt e (elements FA s).
Proof.
intros e s; apply refl_equal.
Qed.
...
assert (_H := elements_spec3 _ s).
set (l := elements FA s) in *; clearbody l; clear s.
assert (H : StronglySorted (fun x y : A => Oset.compare Elt x y = Lt) l).
{
  apply Sorted_StronglySorted; [ | assumption].
  intros x1 x2 x3; apply Oset.compare_lt_trans.
}
clear _H.
induction l as [ | a1 l]; [apply refl_equal | ].
inversion H; clear H; subst.
rewrite Oset.mem_bool_unfold, Oset.nb_occ_unfold.
case_eq (Oset.compare Elt e a1); intro He; [ | apply IHl; assumption | apply IHl; assumption].
assert (H : Oset.nb_occ Elt e l = 0%N).
{
  apply Oset.not_mem_nb_occ.
  case_eq (Oset.mem_bool Elt e l); [ | intros; trivial].
  intro H; apply False_rec.
  rewrite Oset.mem_bool_true_iff in H.
  rewrite Forall_forall in H3.
  generalize (H3 _ H).
  rewrite (Oset.compare_eq_sym _ _ _ He);
  discriminate.
}
rewrite H, N.add_0_r.
apply refl_equal.
...
Proof.
intros l1 l2; rewrite equal_spec; intro a.
rewrite mem_union, 3 mem_mk_set, Oset.mem_bool_app.
apply refl_equal.
Qed.
...
Lemma is_empty_eq :
  forall s1 s2, equal FA s1 s2 = true -> is_empty _ s1 = is_empty _ s2.
Proof.
intros s1 s2 Hs; apply eq_bool_iff; rewrite !is_empty_spec; split; intro H.
- rewrite <- (equal_eq_1 _ _ _ Hs); assumption.
- rewrite (equal_eq_1 _ _ _ Hs); assumption.
Qed.

Lemma union_diag :
  forall s, equal FA (union FA s s) s = true.
Proof.
intro s; rewrite equal_spec; intro x.
rewrite mem_union.
apply Bool.Bool.orb_diag.
Qed.
...
Lemma permut_elements_mk_set : 
  forall l, all_diff l -> Oset.permut Elt l (elements _ (mk_set l)).
Proof.
intro l; induction l as [ | a1 l]; intro H; simpl.
- rewrite elements_empty; constructor 1.
- rewrite all_diff_unfold in H.
  assert (K : exists l1 l2 : list A, 
             elements FA (mk_set l) = l1 ++ l2 /\ 
             elements FA (add FA a1 (mk_set l)) = l1 ++ a1 :: l2).
  {
    apply elements_add_2.
    rewrite mem_mk_set, <- not_true_iff_false, Oset.mem_bool_true_iff.
    intro K; destruct (in_split _ _ K) as [l1 [l2 J]].
    refine (proj1 H a1 _ (refl_equal _)).
    rewrite J; apply in_or_app; right; left; apply refl_equal.
  }
  destruct K as [l1 [l2 [K1 K2]]].
  rewrite K2; constructor 2; [apply Oset.compare_eq_refl | ].
  rewrite <- K1; apply IHl.
  apply (proj2 H).
Qed.

Lemma permut_elements_mk_set_alt : 
  forall s l, Oset.permut Elt l (elements FA s) -> equal _ (mk_set l) s = true.
Proof.
intros s l Hp.
rewrite equal_spec; intro a.
rewrite mem_mk_set, mem_elements, eq_bool_iff.
rewrite !Oset.mem_bool_true_iff; split; intro H.
- refine (Oset.in_permut_in _ H Hp).
- refine (Oset.in_permut_in _ H (Oset.permut_sym Hp)).
Qed.
...
Lemma all_diff_elements : forall s, all_diff (elements FA s).
Proof.
intros s.
...
- inversion Aux; clear Aux; subst. 
  rewrite all_diff_unfold; split; [ | apply IHl; apply H2].
  intros a Ha Ha1; subst a1; apply H1.
  rewrite InA_alt.
