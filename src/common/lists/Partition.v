...
Lemma partition_value_permut :
  forall lv e ls, flat_map (fun x => x) ls =P= flat_map (fun x => x) (partition_value e lv ls).
Proof.
intros lv; induction lv as [ | v1 lv]; 
  intros e ls; simpl; [ apply _permut_refl; intros; apply refl_equal | ].
refine (_permut_trans _ (IHlv e ls) _); [intros; subst; apply refl_equal | ].
destruct (partition_value e lv ls) as [ | ll1 ll]; [apply Pnil | simpl].
assert (Aux := partition_spec3_strong (fun t : A => Oset.eq_bool OVal (e t) v1) ll1).
destruct (List.partition (fun t : A => Oset.eq_bool OVal (e t) v1) ll1) as [l1 l2].
generalize (Aux _ _ (refl_equal _)); clear Aux; intro Aux; simpl.
rewrite List.ass_app; apply _permut_app.
- refine (_permut_trans _ Aux _); [intros; subst; trivial | ].
  apply _permut_swapp; apply _permut_refl; intros; trivial.
- apply _permut_refl; intros; trivial.
Qed.

Lemma partition_value_invariant :
  forall e lv s lls, (forall t1 t2, Oeset.mem_bool OA t1 s = true -> t1 =t= t2 -> e t1 = e t2) ->
                      ...
                      Oeset.mem_bool OA t s2 = true -> False)).
    {
      ...
      simpl in Hs2; simpl in Hs3.
      {
        ...
        - subst s2; right; intros t Ht Kt.
          ...
          rewrite <- Hp, partition_spec1, Oeset.mem_bool_filter, Bool.Bool.andb_true_iff in Ht.
          + rewrite Oeset.mem_bool_true_iff in Ht.
            ...
            rewrite <- (He t t1); [ | | assumption].
            * rewrite (He t t3); [ | | assumption].
              -- rewrite Js3; assumption.
              -- assert (Aux := partition_value_permut lv e (s :: nil)); 
                   rewrite Hs in Aux; simpl in Aux; rewrite <- app_nil_end in Aux.
                 rewrite (Oeset.mem_bool_eq_1 OA _ _ s Ht); apply Oeset.in_mem_bool.
                 rewrite (in_permut_in Aux); apply in_or_app; left; assumption.
            * assert (Aux := partition_value_permut lv e (s :: nil)); 
                rewrite Hs in Aux; simpl in Aux; rewrite <- app_nil_end in Aux.
              rewrite (Oeset.mem_bool_eq_1 OA _ _ s Ht); apply Oeset.in_mem_bool.
              rewrite (in_permut_in Aux); apply in_or_app; left; assumption.
          + intros x1 x2 H Hx; unfold f1.
            apply f_equal2; [apply He; trivial | apply refl_equal].
            rewrite Oeset.mem_bool_true_iff in H; destruct H as [x [Hx1 H]].
            rewrite (Oeset.mem_bool_eq_1 OA _ _ s Hx1).
        ...
        - subst s3; right; intros t Kt Ht.
          ...
          + rewrite Bool.andb_true_iff, Oeset.mem_bool_true_iff in Ht.
            ...
            rewrite Oeset.mem_bool_true_iff in Kt.
            destruct Kt as [t3 [Kt Ht3]].
            replace (e t1) with v3; [assumption | ].
            replace (e t1) with (e t3); [apply sym_eq; apply Js3; assumption | ].
            apply eq_trans with (e t).
            * apply sym_eq; apply He; [ | assumption].
              rewrite (Oeset.mem_bool_eq_1 _ _ _ _ Ht); apply Oeset.in_mem_bool;
              rewrite (in_permut_in IH1); apply in_or_app; left; assumption.
            * apply He; [ | assumption].
              rewrite (Oeset.mem_bool_eq_1 _ _ _ _ Ht); apply Oeset.in_mem_bool;
              rewrite (in_permut_in IH1); apply in_or_app; left; assumption.
          + intros x1 x2 Hx Hx'; unfold f1; apply f_equal2; [ | apply refl_equal].
            apply He; [ | assumption].
            rewrite Oeset.mem_bool_true_iff in Hx.
            destruct Hx as [x [Hx1 Hx]].
            rewrite (Oeset.mem_bool_eq_1 _ _ _ _ Hx1); apply Oeset.in_mem_bool.
            rewrite (in_permut_in IH1); apply in_or_app; left; assumption.
...
Lemma partition_value_eq :
  forall e1 e2 lv s1 s2, 
    (forall t1 t2, Oeset.mem_bool OA t1 s1 = true -> t1 =t= t2 -> e1 t1 = e2 t2) ->
    s1 =PE= s2 ->
...
- destruct (IHlv _ _ He Hs) as [lls [H1 [H2 H]]].
  ...
  + rewrite map_unfold in Hlls1, Hlls2.
    ...
    * injection K; clear K; do 2 intro; subst p p'.
      refine (proj2 (Oeset.partition_permut f1 f2 (l := p1) _ Hp1 Hp2 Hp)).
      intros x y K1 Hxy; unfold f1, f2.
      apply f_equal2; [ | apply refl_equal].
      apply He; [ | assumption].
      assert (Aux := partition_value_invariant).
      rewrite partition_value_unfold in H1; simpl in H1.
      simpl in H1
    * injection K; clear K; do 2 intro; subst p p'.
      apply (proj1 (Oeset.partition_permut f1 f2 (l := p1) (fun x y _ => Hf x y) Hp1 Hp2 Hp)).
...
- destruct (IHlv _ _ He Hs) as [lls [H1 [H2 H]]].
  ...
  + rewrite map_unfold in Hlls1, Hlls2.
    ...
    simpl in K; destruct K as [K | [K | K]].
    * injection K; clear K; do 2 intro; subst p p'.
      apply (proj2 (Oeset.partition_permut f1 f2 (l := p1) (fun x y _ => Hf x y) Hp1 Hp2 Hp)).
    * injection K; clear K; do 2 intro; subst p p'.
      apply (proj1 (Oeset.partition_permut f1 f2 (l := p1) (fun x y _ => Hf x y) Hp1 Hp2 Hp)).
    * apply H; right; assumption.
Qed.
               ...
               {{{Fset.mk_set FVal (map e2 s2)}}}).
{
  ...
  - assert (Kt : Oeset.mem_bool OA t s2 = true).
    {
      rewrite <- (Oeset.permut_mem_bool_eq t Hs), Oeset.mem_bool_true_iff.
...
- rewrite H2; clear s1 s2 H H1 H2.
  ...
  + destruct s2; [ | inversion Hs].
    apply IHlls; intros; apply Hll; right; assumption.
  + destruct s2 as [ | t2 s2]; [assert (Ks := Oeset.permut_sym Hs); inversion Ks | ].
    simpl; apply f_equal.
    apply IHlls; intros; apply Hll; right; assumption.
...
Lemma in_partition_list_expr_not_empty :
  forall le s,
    (forall e, In e le -> forall t1 t2, t1 =t= t2 -> e t1 = e t2) ->
    forall p, In p (partition_list_expr s le) -> p <> nil.
Proof.
intros le s Hle p Hp.
destruct (partition_list_expr_invariant le p Hle (refl_equal _)) 
    as [H1 [H2 [H3 [H4 [H5 H6]]]]].
rewrite partition_list_expr_partition_list_expr2 in Hp; [ | assumption].
rewrite filter_In in Hp; destruct Hp as [_ Hp].
destruct p; [discriminate Hp | discriminate].
Qed.
...
rewrite <- (Oeset.permut_mem_bool_eq _ H).
...
- revert H'; apply _permut_map.
  ...
  + assert (Hq1 := quick_sorted OA (a :: lk1)).
    ...
    apply Oeset.permut_trans with (a :: lk1).
    * apply Oeset.permut_sym; apply quick_permut.
    * {
        apply Oeset.permut_trans with (a :: lk2).
        - apply Oeset.permut_cons; [apply Oeset.compare_eq_refl | ].
          apply Oeset.permut_trans with (quicksort OA lk1).
          + apply quick_permut.
          + apply Oeset.permut_trans with (quicksort OA lk2); 
              [ | apply Oeset.permut_sym; apply quick_permut].
            apply Oeset.permut_refl_alt; apply Hlk.
        - apply quick_permut.
      }
  + assumption.
  + assumption.
...
- apply _permut_app; [apply H' | ].
  apply Oeset.permut_cons; [ | apply Pnil].
  apply Oeset.compare_eq_refl.
Qed.
...
- rewrite comparelA_unfold in Hs.
  ...
  rewrite 2 flg_app.
  apply Oeset.permut_trans with (flg le (flg le acc s2) (a1 :: nil)).
  + apply flg_eq_2.
    * apply IHs1; trivial.
    * apply all_diff_keys_flg; trivial.
    * apply all_diff_keys_flg; trivial.
  + set (acc2 := flg le acc s2).
    apply Oeset.permut_refl_alt; simpl.
    ...
    apply Oeset.permut_trans with lk1; [apply Oeset.permut_sym; apply quick_permut | ].
    apply Oeset.permut_trans with lk2; [ | apply quick_permut].
    apply Oeset.permut_refl_alt; apply Hlk.
Qed.    
...
- destruct (insert_in_one_group 
  ...
  * destruct H1 as [ll [H1 H2]].
    ...
    apply Oeset.permut_trans with (lg1' ++ (k', a2 :: lk') :: ll ++ (k, a1 :: lk) :: lg2).
    {
  ...
  * destruct H1 as [ll [H1 H2]]; rewrite H1, H2.
    apply Oeset.permut_trans with (lg1 ++ (k, a1 :: lk) :: ll ++ (k', a2 :: lk') :: lg2').
    {
  ...
  * destruct H1 as [H1 [H2 H3]]; injection H1; clear H1; do 2 intro; subst k' lk' lg1' lg2'.
    apply Oeset.permut_trans with (lg1 ++ (k, a1 :: a2 :: lk) :: lg2).
    {
      rewrite insert_in_one_group_alt.
      - apply Oeset.permut_refl.
      - assumption.
    ...
    }
    {
      ...
      - apply Oeset.permut_refl_alt.
        ...
        + rewrite comparelA_unfold, comparelA_eq_refl; [ | intros; apply Oeset.compare_eq_refl].
          ...
          rewrite Aux; [trivial | ].
          refine (Oeset.permut_trans _ (quick_permut _ _)).
          apply Oeset.permut_sym; refine (Oeset.permut_trans _ (quick_permut _ _)).
...
- rewrite (insert_in_groups_weak (oeset_of_oset (mk_olists OVal))
  ...
  + rewrite (insert_in_groups_weak 
    ...
    * unfold oeset_of_oset, Oeset.compare in Ha.
      ...
      apply Oeset.permut_refl_alt; rewrite comparelA_unfold.
    ...
    * {
        ...
        - apply Oeset.compare_eq_refl.
        - rewrite <- app_nil_end; apply Oeset.permut_refl.
      }
    * {
        apply (Pcons 
               (map (fun e : A -> value => e a2) le, a2 :: nil)
...
- revert acc1 acc2 Hacc Hacc1 Hacc2.
  ...
  + inversion H; subst.
    ...
    *
      {
        apply Oeset.permut_trans with (flg le acc1 (l0 ++ a2 :: b2 :: l3)).
        ...
        - apply Oeset.permut_trans with  (flg le acc1 (l0 ++ b2 :: a2 :: l3)).
