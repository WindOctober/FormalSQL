...
Lemma quicksort_1 :
  forall x, quicksort (x :: nil) = x :: nil.
Proof.
intros x; rewrite quicksort_equation; simpl.
rewrite quicksort_equation; apply refl_equal.
Qed.
...
functional induction (partition e l) 
    as [ | l a' l' l_eq_a'_l' IH l1' l2' H a'_lt_e | l a' l' l_eq_a'_l' IH l1' l2' H _H _ ].
contradiction.
...
assert (IH := quick_sorted_aux4 l e); rewrite Hl in IH; apply (IH a Ha).
assert (IH := quick_sorted_aux4 l e); rewrite Hl in IH; apply IH.
simpl; intros a [Ha | Ha]; [subst a1; rewrite Oeset.compare_lt_gt, He; discriminate | ].
assert (IH := quick_sorted_aux4 l e); rewrite Hl in IH; apply (IH a Ha).
Qed.
...
- apply quick_sorted_aux1; trivial.
  + intros a a_in_ql1.
    ...
    {
      rewrite (Oeset.permut_mem_bool_eq a (quick_permut l1)).
  ...
  + intros a a_in_ql2.
    ...
    {
      rewrite (Oeset.permut_mem_bool_eq a (quick_permut l2)).
      rewrite Oeset.mem_bool_true_iff.
      exists a; split; [apply Oeset.compare_eq_refl | assumption].
    } 
    rewrite Oeset.mem_bool_true_iff in K.
  ...
  + assert (e1_eq_a1 : Oeset.compare OA e1 a1 = Eq).
    {
      ...
      {
        rewrite (Oeset.permut_mem_bool_eq _ P).
...
Proof.
intros; apply Oeset.permut_mem_bool_eq.
apply quick_permut.
Qed.
...
Proof.
intros x l1; apply sym_eq; apply Oeset.permut_nb_occ; apply quick_permut.
Qed.
