...
Lemma if_mul :
  forall (b1 b2 : bool),
   ((if (andb b1 b2) then 1 else 0) = (if b1 then 1 else 0) * (if b2 then 1 else 0))%N.
Proof.
intros [ | ] [ | ]; apply refl_equal.
Qed.
...
Lemma if_eq_true :
  forall (A : Type) (b : bool) (a1 a2 : A), b = true -> (if b then a1 else a2) = a1.
Proof.
intros; subst; trivial.
Qed.

Lemma if_eq_false :
  forall (A : Type) (b : bool) (a1 a2 : A), b = false -> (if b then a1 else a2) = a2.
Proof.
intros; subst; trivial.
Qed.
...
Lemma if_same :
  forall (A : Type) (b : bool) (a : A), (if b then a else a) = a.
Proof.
...
Lemma In_eq_2 : forall (A : Type) (a : A) (l1 l2 : list A), l1 = l2 -> In a l1 -> In a l2.
Proof.
...
Lemma N_le_1_and_diff_0 :
  forall (n : N), (n <= 1 -> n <> 0 -> n = 1)%N.
Proof.
intros n Hn Kn; destruct n as [ | p]; [apply False_rec; apply Kn; apply refl_equal | ].
destruct p as [p | p | ]; [ | | apply refl_equal].
- destruct p as [p | p | ]; apply False_rec; apply Hn; apply refl_equal.
- destruct p as [p | p | ]; apply False_rec; apply Hn; apply refl_equal.
Qed.
